...
def assign_number_to_hexes(spiral_order, number_order):
    hex_assignment = []
    number_index = 0
    print(spiral_order, number_order)
    ...
    for i in range(len(spiral_order)):
        if spiral_order[i] == 'D':
            ...
            if number_index < len(number_order):
                ...
                hex_assignment.append({'resource': spiral_order[i], 'number': number_order[number_index], 
                    'dots': dots, 'dots_as_stars': get_dots_as_stars(dots), 
                    # 'position': position_order[i]
                    })
                number_index += 1
            else:
...
def generate_complete_board(number_of_players):
    ...
    rows = [['O', 'L', 'G'], ['O', 'W', 'B', 'O'], ['W', 'L', 'W', 'B', 'L'], ['G', 'B', 'W', 'L'], ['D', 'O', 'G']]
...
board = generate_complete_board(3)
